...
List = dir(fullfile(Base, '2022-12-12 CT(150~328)', '*.*'));
List = List([List.isdir]);
SubFolder = {List.name};
SubFolder(ismember(SubFolder, {'.', '..'})) = [];
Source_dir1 = cellfun(@(c)[Base '2022-12-12 CT(150~328)\' c '\' c '\'],SubFolder,'uni',false);
...
% load excel file
[AP_Data_Label] = readcell(fullfile(Base, 'excel/newct2.xlsx'));
...
for i = 1 : size(Dicom2,2)
    for j = 1 : size(Dicom2(i).ImageAdjust,1)
        if WriteOrNot == 1
            Temp = [Base,'newctpng2\',Dicom2(i).JPGImageName{j,:}];
            
            if ~exist([Base,'newctpng2'], 'dir')
                mkdir([Base,'newctpng2'])
            end    
